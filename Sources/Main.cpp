...
protected:
	...
	void LoadLevel(const std::string& sMap, const def::vi2d& vSize, const std::unordered_map<char, TileType>& mapLookUp)
	{
		CreateLevel(vSize);
	...
	void UpdateCollision(float fDeltaTime)
	{
		...
		if (GetTile(vNewPlayerPos + 1.0f) == TileType::Coin)
		{
			SetTile(vNewPlayerPos + 1.0f, TileType::Empty);
			nScore++;
		}

		if (vPlayerVel.x <= 0.0f)
		{
			if (GetTile(def::vi2d(vNewPlayerPos.x, vPlayerPos.y)) != TileType::Empty || GetTile(def::vi2d(vNewPlayerPos.x, vPlayerPos.y + 0.9f)) != TileType::Empty)
			{
		...
		{
			if (GetTile(def::vi2d(vNewPlayerPos.x + 1.0f, vPlayerPos.y)) != TileType::Empty || GetTile(def::vi2d(vNewPlayerPos.x + 1.0f, vPlayerPos.y + 0.9f)) != TileType::Empty)
			{
